        ...
        if (event == "telemetry") {
         ...
         bool Car_right_close = false;
          for(int i =0 ; i<sensor_fusion.size(); i++)
          {
            ...
            car_dir = car_left;
            if (d>8) car_dir = car_right;
            else if (d>4)car_dir = car_middle;
            double vx = sensor_fusion[i][3];
			double vy = sensor_fusion[i][4];
			double predict_speed = sqrt(vx*vx+vy*vy);
			double predict_car_s = sensor_fusion[i][5];

            predict_car_s += ((double)prev_size*0.02*predict_speed);

            if(car_dir==lane)
            {
             
              Car_Collision |= predict_car_s > car_s && (predict_car_s - car_s) < 30;	
              //Car_Collision = true;
            }
            else if((car_dir-lane)==-1)
            {
              if((car_s+30) > predict_car_s  && (car_s-30) < predict_car_s)
                Car_left_close = true;
            }
             else if((car_dir-lane)==1)
            {
